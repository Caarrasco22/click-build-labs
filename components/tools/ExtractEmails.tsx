      ...
      {result.length > 0 && (
        <div className="space-y-2">
          <p className="text-sm text-zinc-500 dark:text-zinc-400">
            Found {result.length} email{result.length !== 1 ? 's' : ''} ({result.length !== [...new Set(result)].length ? 'duplicates removed' : 'all unique'})
          </p>
          <div className="p-4 rounded-lg border border-green-200 dark:border-green-900 bg-green-50 dark:bg-green-900/20 space-y-2">
            {result.map((email, i) => (
              <div key={i} className="p-2 bg-white dark:bg-zinc-800 rounded font-mono text-sm">
                {email}
              </div>
            ))}
          </div>
